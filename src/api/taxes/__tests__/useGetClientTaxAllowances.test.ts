...
import { transformClientTaxAllowances, GetClientTaxAllowancesData } from '../useGetClientTaxAllowances';
...
describe('Tax Allowances Data Transformation', () => {
  ...
  describe('Complete ISA Data Transformation', () => {
    it('should transform complete ISA data correctly', () => {
      ...
      const result = transformClientTaxAllowances(mockData as GetClientTaxAllowancesData);
      ...
      expect(result.isaAllowance).toEqual({
        ...
        children: [{ code: StandardSolutionTaxWrapper.CashISA }, { code: StandardSolutionTaxWrapper.StocksAndSharesISA }],
        portfolioTypes: [
          { id: "cash-type", code: "CASH", name: "Cash" },
          { id: "stocks-type", code: "STOCKS", name: "Stocks" }
        ]
      ...
      const cisaChild = result.childAllowances.find(child => child.taxWrapperCode === StandardSolutionTaxWrapper.CashISA);
      ...
      const ssisaChild = result.childAllowances.find(child => child.taxWrapperCode === StandardSolutionTaxWrapper.StocksAndSharesISA);
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle missing tax wrapper name gracefully', () => {
      ...
      const cisaChild = result.childAllowances.find(child => child.taxWrapperCode === StandardSolutionTaxWrapper.CashISA);
      expect(cisaChild!.taxWrapperName).toBe(StandardSolutionTaxWrapper.CashISA); // Should fallback to code
    });
