 ...
 */
export const useTaxWrapperPortfolioMapping = (taxAllowances: ClientTaxAllowance[]) => {
  const { selectedContactId } = useGetContractIdData();
  const { data: contactInfo } = useGetContactInfo(false, selectedContactId);
  const { t } = useModifiedTranslation();
  
  const portfolios = useMemo(() => contactInfo?.portfolios || [], [contactInfo?.portfolios]);
  ...
  const mappedTaxAllowances: TaxWrapperWithPortfolios[] = useMemo(() => {
    return taxAllowances.map(allowance => {
      ...
      const mappedPortfolios = portfolios.filter(portfolio => {
        // Match portfolio type codes between tax wrapper and actual portfolios
        return allowance.portfolioTypes.some(wrapperPortfolioType => 
          portfolio.typeCode === wrapperPortfolioType.code
        );
      }).map(portfolio => ({
        id: portfolio.id,
        code: portfolio.shortName || t("utils.portfolio.fallbackCode", { id: portfolio.id }),
        name: portfolio.name || t("utils.portfolio.fallbackName", { id: portfolio.id }),
      }));
