...
import { MappedPortfolio } from "hooks/useTaxWrapperPortfolioMapping";
import { Tooltip } from "react-tooltip";
import { QuestionmarkIcon } from "views/overview/components/PortfolioInfoCard";

// Progress Bar Component
const ProgressBar = ({ used, total, color = "bg-green-600" }: { used: number; total: number; color?: string }) => {
    ...
    <>
      ...
      <div className="md:hidden">
        ...
        <div className="text-base font-medium text-gray-900">
          {t("numberWithCurrencyNoDecimals", { value: totalAmount, currency })}
        </div>
      </div>
      ...
      <div className="flex justify-between items-start pt-1">
        ...
        <div className="text-left md:text-center">
          ...
          <div className="text-base font-medium text-gray-900">
            {t("numberWithCurrencyNoDecimals", { value: usedAmount, currency })}
          </div>
        </div>
        ...
        <div className="text-right">
          ...
          <div className="text-base font-semibold text-green-600">
            {t("numberWithCurrencyNoDecimals", { value: remainingAmount, currency })}
          </div>
        </div>
      </div>
      ...
      <div className="p-3 mt-3 bg-gray-50 rounded-lg border border-gray-200">
        {mappedPortfolios.length > 0 ? (
          mappedPortfolios.map((portfolio, index) => (
            <div key={portfolio.id} className={`flex justify-between items-center ${index > 0 ? 'mt-2 pt-2 border-t border-gray-200' : ''}`}>
          ...
          <div className="flex justify-between items-center">
            <span className="text-sm text-gray-500">{t("taxesPage.noPortfoliosAvailable")}</span>
            <Button
              variant="Primary"
              size="xs"
              disabled
      ...
      {isConstrainedByParent && showTooltip && parentRemainingAmount !== undefined && (
        <Tooltip
